...
class PlantService:
    ...
    async def get_plant(self, plant_id: ObjectId) -> Optional[Plant]:
        return await self.plant_repo.find_by_id(plant_id)

    async def list_plants(self) -> List[Plant]:
        return await self.plant_repo.list()

    async def update_plant(self, plant_id: ObjectId, update_data: dict) -> Optional[Plant]:
        return await self.plant_repo.update(plant_id, update_data)

    async def delete_plant(self, plant_id: ObjectId) -> bool:
