...
class ChatMessageService:
    ...
    async def get_message(self, message_id: ObjectId) -> Optional[ChatMessage]:
        return await self.chat_repo.find_by_id(message_id)

    async def list_messages_by_plant(self, plant_id: ObjectId) -> List[ChatMessage]:
        return await self.chat_repo.list_by_plant(plant_id)

    async def delete_message(self, message_id: ObjectId) -> bool:
        return await self.chat_repo.delete_by_id(message_id)

    async def delete_messages_by_plant(self, plant_id: ObjectId) -> int:
